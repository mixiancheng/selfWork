...
Socket* Socket::accept() {
    ...
    #else
        #if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)//zhangqi, 20130309, avoid type error of arg for accept
            socklen_t addrSize = sizeof(incoming_addr);//zhangqi
        #else
...
void Socket::send(const void* buffer, size_t size) {
    ...
    CCLOG("type is CLIENT");
    
    if(_protocol == UDP)
        return sendTo(buffer, size, _hostTo, _portTo);
    
    CCLOG("protocol not UDP");
...
//    int status = recv(_socketHandler, (char*)buffer, bufferSize, 0);
    // 修改最后一个参数为 MSG_WAITALL, 强制等到读够 bufferSize 长度的数据后才返回, modified by zhangqi, 20130711
    int status = recv(_socketHandler, (char*)buffer, bufferSize, MSG_WAITALL);
